...
def load_eeg_data_for_cnn(
    ...
    """
    Same data loading as before, but only returning X, y for training.
    """
...
def build_cnn_best(input_shape, num_classes):
    """
    Best CNN params found:
      conv_filters=4, dense_units=16, dropout_rate=0.3, learning_rate=0.0001
    """
    conv_filters = 4
    dense_units = 16
    dropout_rate = 0.3
...
def main():
    ...
    X, y = load_eeg_data_for_cnn("processed", time_points=128, per_participant_norm=True)
    if len(X) == 0:
        return
    ...
    model = build_cnn_best((X.shape[1], X.shape[2]), num_classes)
    model.fit(X_train, y_train, epochs=30, batch_size=8, validation_split=0.2, verbose=1)
    loss, acc = model.evaluate(X_test, y_test, verbose=0)
    print(f"\n[CNN Best Params] Test Accuracy: {acc:.4f}")
